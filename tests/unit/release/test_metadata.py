...
from development_foundation.release import ReleaseError, build_metadata, preflight
...
def test_release_metadata_binds_package_schema_and_skill_identities() -> None:
    ...
    assert {component.version for component in metadata.components} == {"2.0.0"}
...
def test_preflight_requires_exact_clean_source_and_final_receipt(
    ...
    repository = Path(__file__).resolve().parents[3]
    receipt_root = tmp_path / "foundation-receipts"
    receipt_root.mkdir()
