...
class FoundationIdentity(BaseModel):
    """Pinned identity of one compatible foundation release."""
    ...
    def validate_version(cls, value: str) -> str:
        if fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?", value) is None:
            raise ValueError("foundation.version must be an immutable semantic version")
        return value
    ...
    def validate_digest(cls, value: str) -> str:
        if fullmatch(r"sha256:[0-9a-f]{64}", value) is None:
            raise ValueError("foundation.digest must be a sha256 identity")
        return value
