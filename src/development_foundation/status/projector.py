...
def project_status(
    ...
    plans_by_specification = {
        PurePosixPath(plan.specification).stem: plan for plan in plans
    }
    members_by_plan = {member.plan_id: member for member in evidence.members}
    integrations = {
        integration.member_commit: integration.batch_commit
        for integration in evidence.integrations
    }
    deliveries = {
        delivery.batch_commit: delivery.delivery_commit
    ...
    for clause in review.clauses:
        ...
        plan = (
            plans_by_specification.get(owner.id.rpartition("/")[2])
            if owner is not None
            else None
        )
        member = members_by_plan.get(plan.id) if plan is not None else None
        batch_commit = (
            integrations.get(member.commit)
            if member is not None and member.ready
            else None
        )
        delivery_commit = (
            deliveries.get(batch_commit) if batch_commit is not None else None
        )
    ...
    return StatusProjection(
        review=review.id, state=parent_state, clauses=tuple(clauses)
    )
