...
from development_foundation.change.models import FrozenModel, require_relative_path
...
class GitInventoryProvider:
    ...
    def read(self, repository: Path) -> RepositoryInventory:
        ...
        paths = frozenset(
            item for item in completed.stdout.decode().split("\0") if item
        )
...
class ReferenceResolver:
    ...
    def resolve(self, reference: str) -> Path:
        ...
        if not self._inventory.contains(normalized):
            raise ValueError(
                f"reference is absent from repository inventory: {reference}"
            )
