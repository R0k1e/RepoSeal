"""Typed Signetum v2 repository configuration boundary."""
...
class SignetumIdentity(BaseModel):
    ...
    def validate_template_version(cls, value: str) -> str:
        if fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?", value) is None:
            raise ValueError("signetum.template_version must be semantic")
        return value
...
def load_manifest(path: str | Path) -> RepositoryManifest:
    ...
    source = Path(path)
    if source.name != "signetum.toml":
        raise ManifestError("active configuration must be named signetum.toml")
    try:
